...
export default function SymptomInput() {
  ...
  const diagnose = async () => {
    ...
    try {
      // Use client-side diagnosis for reliable operation
      const apiResponse = await DiagnosisAPI.analyzeSymptoms(trimmedSymptoms);

      if (!apiResponse.success || !apiResponse.data) {
        throw new Error(apiResponse.error || 'Diagnosis analysis failed');
      }

      // Save the diagnosis result to history
      await addDiagnosis(apiResponse.data);

      // Navigate directly to result screen
      router.push(`/diagnosis/result?diagnosisId=${apiResponse.data.id}`);
    } catch (error) {
